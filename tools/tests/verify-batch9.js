...
/* admin: exams/holiday/votes/invoice/sms/voice/lfphoto */
{
  ...
  run('db=DB.load(); db.teacherVotes={T001:12}; DB.save(db); renderVotes();');
  ok('tally renders', w.document.getElementById('voteRows').textContent.includes('Shaibu Memunat') && w.document.getElementById('voteRows').textContent.includes('12'));
  run('crownWinner();');
  const tot = w.__DB.load().teacherOfTerm;
  ok('highest vote crowned + reset', tot.name === 'Shaibu Memunat' && tot.votes === 12 && Object.keys(w.__DB.load().teacherVotes).length === 0);
  run('crownWinner();');
  ok('no-vote crown blocked', w.__DB.load().teacherOfTerm.name === 'Shaibu Memunat');
